...
static bool lift_push(DSECtx *ctx, const cs_insn *insn, const cs_x86 *x86, const InsnAux *aux, const DseArch *arch) {
	...
	SymExpr *val = get_operand_expr(ctx, src, natw, aux, arch);
	if (val) {
		uint8_t sw = val->width;
	...
	if (esp) {
		SymExpr *ne = sym_expr_sub(esp, sym_expr_const(natw / 8, natw));
		dse_set_reg(ctx, REG_RSP, ne, natw);
	}
	return true;
}
...
static bool lift_pop(DSECtx *ctx, const cs_insn *insn, const cs_x86 *x86, const InsnAux *aux, const DseArch *arch) {
	...
	SymExpr *val = dse_load_mem(ctx, aux, natw, arch->big_endian);

	SymExpr *esp = dse_read_rid_fit(ctx, aux, REG_RSP, natw, natw);
	if (esp) {
		SymExpr *ne = sym_expr_add(esp, sym_expr_const(natw / 8, natw));
		dse_set_reg(ctx, REG_RSP, ne, natw);
	}
